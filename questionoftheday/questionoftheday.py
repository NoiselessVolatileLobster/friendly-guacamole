...
class QuestionData(BaseModel):
    # NEW: Mandatory ID for duplicate checking and export/import
...
class SuggestionModal(discord.ui.Modal, title="Submit a Question of the Day"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        # ID is automatically generated by the QuestionData default factory
        new_q = QuestionData(
...
class ApprovalView(discord.ui.View):
    ...
    async def approve_callback(self, interaction: discord.Interaction):
        ...
        embed.set_footer(text=f"Processed by: {interaction.user.name} | Question ID: {self.question_id.split('-')[0]}")
...
class QuestionOfTheDay(commands.Cog):
    ...
    async def _post_scheduled_question(self, schedule_id: str, schedule: Schedule):
        ...
        for qid, qdata in eligible_q_data.items():
            try:
                ...
                # Also ensure the question ID in the dictionary key matches the object's ID field
                qdata['id'] = qid
        ...
        if not_asked:
            ...
            # Use the ID directly from the selected object
        ...
        if selected_q.suggested_by:
            ...
            embed.set_footer(text=f"Suggested by {suggested_by_text} | ID: {selected_qid.split('-')[0]}")
    ...
    async def add_question_to_data(self, question: QuestionData):
        """Adds a new question to the global questions dict using its inherent UUID."""
    ...
    async def _notify_new_suggestion(self, qid: str, question: QuestionData):
        ...
        embed.set_footer(text=f"Question ID: {qid.split('-')[0]}") # Show short ID for simplicity
    ...
    async def qotd_suggest_list(self, ctx: commands.Context):
        ...
        for qid, qdict in pending_questions_data.items():
            try:
                ...
                qdict['id'] = qid # Ensure ID is set for validation
    ...
    async def qotd_suggest_approve(self, ctx: commands.Context, short_qid: str, list_id: str):
        ...
        try:
            # Re-parse datetimes and ensure ID is included before validation
    ...
    async def qotd_import(self, ctx: commands.Context, list_id: str):
        ...
        The file should be attached to the command message. It checks for duplicate IDs 
        and preserves all question metadata, including `suggested_by_id` and `added_on`.
        """
        ...
        async with self.config.questions() as global_questions:
            for item in questions_list:
                ...
                # --- NEW: Check for existing ID ---
                imported_id = item.get("id")
                
                # If an ID is present and it already exists in the global config, skip
                ...
                # --- Final ID Determination ---
                # Use imported ID if valid string, otherwise generate a new one
                final_id = imported_id if imported_id and isinstance(imported_id, str) else str(uuid.uuid4())
                
                # Highly unlikely, but a final check for UUID V4 collision
                if final_id in global_questions:
                ...
                new_q = QuestionData(
                    ...
                    status="not asked",
                    added_on=added_on,
                    # Note: last_asked is not imported to prevent immediate re-asking
                )
                
                # CRITICAL: Double serialize to guarantee only JSON primitives (strings for datetime) are saved.
                serialized_q = json.loads(new_q.model_dump_json())
                
                global_questions[final_id] = serialized_q
                imported_count += 1

        await ctx.send(f"Import complete. Imported **{imported_count}** questions into list **{lists_data[list_id]['name']}**. Skipped {skipped_count} items (header identifiers/corrupt data) and **{duplicate_count}** questions that already existed based on their unique ID.")
    ...
    async def qotd_export(self, ctx: commands.Context, list_id: str):
        ...
        for qid, qdict in all_questions.items():
            if qdict.get('list_id') == list_id:
                ...
                export_data.append({
                    "id": qid, # NEW: Include the unique question ID
