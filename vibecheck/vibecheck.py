...
class Ephemeral(commands.Cog):
    ...
    def __init__(self, bot):
        ...
        self.config.register_guild(
            ...
            # --- NEW MEMBER XP CONFIGURATION ---
            newmember_xp={
                "enabled": False,
                "time": 0, # seconds (from minutes input)
                "threshold": 0, # message count
                "xp": 0
            },
            # --- END NEW MEMBER XP CONFIGURATION ---
        ...
        self.config.register_member(
            ...
            xp_granted=False, # Track if new member xp has been given
        )
        self.timers = {}
    ...
    async def _award_xp(self, guild: discord.Guild, user: discord.Member, amount: int):
        """Attempts to award XP via a LevelUp cog."""
        levelup = self.bot.get_cog("LevelUp")
        if not levelup:
            print(f"Ephemeral WARNING: LevelUp cog not found. Cannot award {amount} XP to {user.id}.")
            return False

        # Attempt to find a suitable method to add XP. 
        # Cogs vary, so we check for common method signatures.
        
        # Method 1: Common API for Vertyco's LevelUp or similar
        # often: .add_xp(guild_id, user_id, amount)
        if hasattr(levelup, "add_xp"):
            try:
                if asyncio.iscoroutinefunction(levelup.add_xp):
                    await levelup.add_xp(guild.id, user.id, amount)
                else:
                    levelup.add_xp(guild.id, user.id, amount)
                return True
            except Exception as e:
                print(f"Ephemeral ERROR: Failed to call add_xp on LevelUp cog: {e}")

        # Method 2: 'api' submodule
        if hasattr(levelup, "api") and hasattr(levelup.api, "add_xp"):
            try:
                if asyncio.iscoroutinefunction(levelup.api.add_xp):
                    await levelup.api.add_xp(guild.id, user.id, amount)
                else:
                    levelup.api.add_xp(guild.id, user.id, amount)
                return True
            except Exception as e:
                print(f"Ephemeral ERROR: Failed to call api.add_xp on LevelUp cog: {e}")
                
        # Method 3: 'give_xp' 
        if hasattr(levelup, "give_xp"):
            try:
                if asyncio.iscoroutinefunction(levelup.give_xp):
                    await levelup.give_xp(guild.id, user.id, amount)
                else:
                    levelup.give_xp(guild.id, user.id, amount)
                return True
            except Exception as e:
                print(f"Ephemeral ERROR: Failed to call give_xp on LevelUp cog: {e}")

        print(f"Ephemeral ERROR: Could not find a compatible 'add_xp' method in the loaded LevelUp cog.")
        return False
    ...
    async def _handle_activation(self, message: discord.Message, settings: dict, guild: discord.Guild, user: discord.Member):
        ...
        try:
            ...
            await self.config.member(user).set({
                ...
                "message_count": 0,
                "is_ephemeral": True,
                "xp_granted": False # Reset xp grant status on new start
            })
    ...
    async def on_message(self, message: discord.Message):
        ...
        if is_ephemeral:
            ...
            if msg_len >= threshold:
                ...
                # --- NEW MEMBER XP REWARD CHECK ---
                xp_settings = settings.get("newmember_xp", {})
                if (xp_settings.get("enabled") and 
                    not member_data.get("xp_granted", False) and 
                    new_count >= xp_settings.get("threshold", 999999)):
                    
                    # Check time since JOIN
                    join_time = member.joined_at or datetime.utcnow()
                    if join_time.tzinfo:
                        join_time = join_time.replace(tzinfo=None) # Make naive
                    
                    time_since_join = datetime.utcnow() - join_time
                    xp_time_limit = timedelta(seconds=xp_settings.get("time", 0))

                    if time_since_join <= xp_time_limit:
                        # User was fast enough!
                        xp_amount = xp_settings.get("xp", 0)
                        if await self._award_xp(guild, member, xp_amount):
                            await self.config.member(member).xp_granted.set(True)
                            await self._log_event(guild, f"✨ **XP Awarded:** {member.mention} reached {new_count} messages within {timedelta_to_human(time_since_join)} of joining! Awarded {xp_amount} XP.")
                        else:
                            await self._log_event(guild, f"⚠️ **XP Error:** {member.mention} qualified for {xp_amount} XP, but LevelUp integration failed.")
                    else:
                        # Too slow, mark as granted=True anyway so we don't check again (optional, but saves processing)
                        # Actually, better to leave it false in case admin extends time? 
                        # No, let's leave it so we don't spam checks if they chat a lot.
                        # But wait, if they chat more, new_count increases. If we don't mark it, we check every msg.
                        # But 'time_since_join' only gets larger. So they will never qualify later.
                        pass 

                # --- END NEW MEMBER XP REWARD CHECK ---
    ...
    @ephemeralset.command(name="newmemberxp")
    @checks.admin_or_permissions(manage_guild=True)
    async def ephemeralset_newmemberxp(self, ctx: commands.Context, time: int, threshold: int, xp: int):
        """
        Sets the New Member XP Reward parameters.

        <time>: Time in MINUTES from joining the server to complete the goal.
        <threshold>: The number of messages (vibe) required to get the XP.
        <xp>: The amount of XP to grant in LevelUp.

        Example: [p]ephemeralset newmemberxp 120 100 500
        (Within 120 minutes of joining, if they reach 100 messages, grant 500 XP)
        """
        ...
        await self.config.guild(ctx.guild).newmember_xp.set(config_data)
        
        msg = (f"✅ **New Member XP Configured:**\n"
               f"Time Limit: {time} minutes (from join)\n"
               f"Message Threshold: {threshold} messages\n"
               f"XP Reward: {xp} XP")
               
        # Warning if threshold is higher than the graduation threshold (user would leave mode before getting XP)
        current_threshold = await self.config.guild(ctx.guild).messages_threshold()
        if threshold > current_threshold:
            msg += f"\n\n⚠️ **WARNING:** Your XP message threshold ({threshold}) is higher than the Ephemeral graduation threshold ({current_threshold}). Users will graduate and leave Ephemeral mode before they can earn this XP reward!"
    ...
    async def ephemeralset_view(self, ctx: commands.Context):
        ...
        # New Member XP Info
        xp_s = settings.get("newmember_xp", {})
        xp_val = "Disabled"
        if xp_s.get("enabled"):
            xp_val = (f"Time: {xp_s.get('time')//60} mins\n"
                      f"Msgs: {xp_s.get('threshold')}\n"
                      f"Reward: {xp_s.get('xp')} XP")
        embed.add_field(name="New Member XP", value=xp_val, inline=False)
